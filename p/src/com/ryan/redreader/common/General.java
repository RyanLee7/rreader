...
import com.ryan.redreader.fragments.ErrorPropertiesDialog;
...
public final class General {
	 ...
	 * @throws IOException
	 */
	public static void moveFile(final File src, final File dst)
			throws IOException {

		if (!src.renameTo(dst)) {

			// renameTo��ʱ����Ч����ʱ��ֱ��COPY��ȥ����ɾ��ԭ�ļ�
			copyFile(src, dst);
	...
	public static void copyFile(final InputStream fis, final File dst)
			throws IOException {
		final FileOutputStream fos = new FileOutputStream(dst);
		copyFile(fis, fos);
	}
	 ...
	 * @throws IOException
	 */
	public static void copyFile(final InputStream fis, final OutputStream fos)
			throws IOException {
	...
	public static boolean isTablet(final Context context,
		...
		final PrefsUtility.AppearanceTwopane pref = PrefsUtility
				.appearance_twopane(context, sharedPreferences);
	...
	public static RRError getGeneralErrorForFailure(Context context,
			RequestFailureType type, Throwable t, StatusLine status) {
		...
		return new RRError(context.getString(title),
				context.getString(message), t, status);
	}
	...
	public static void showResultDialog(final Activity context,
			final RRError error) {
		new Handler(Looper.getMainLooper()).post(new Runnable() {
			public void run() {
				...
				alertBuilder.setNegativeButton(R.string.button_moredetail,
		...
		try {
			...
			try {
				...
				if (urlMatcher.find()) {
					...
					final String path = urlMatcher.group(3).length() == 0 ? null
							: "/" + urlMatcher.group(3);
					final String query = urlMatcher.group(4).length() == 0 ? null
							: urlMatcher.group(4);
					final String fragment = urlMatcher.group(5).length() == 0 ? null
							: urlMatcher.group(5);
					...
					try {
						...
						if (path != null && path.contains(" ")) {
							return new URI(scheme, authority, path.replace(" ",
									"%20"), query, fragment);
						} else {
							return null;
						}
