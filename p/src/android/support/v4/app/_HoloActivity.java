...
public abstract class _HoloActivity extends Watson implements
		SuperStartActivity, OnCreatePanelMenuListener, OnPreparePanelListener,
	...
	public static final class Holo implements Parcelable {
		 ...
		 * 实现Serializable接口是Java
		 * SE本身就支持的，而Parcelable是Android特有的功能，效率比实现Serializable接口高
		 * ，而且还可以用在IPC中。实现Serializable接口非常简单
		 * ，声明一下就可以了，而实现Parcelable接口稍微复杂一些，但效率更高，推荐用这种方法提高性能。
		 */
		//
		...
		private Holo(Parcel source) {
			...
			windowFeatures = source.readParcelable(SparseIntArray.class
					.getClassLoader());
		}
	...
	public void addOnWindowFocusChangeListener(
		...
		synchronized (mOnWindowFocusChangeListeners) {
			...
			mOnWindowFocusChangeListeners
					.add(new WeakReference<OnWindowFocusChangeListener>(
							listener));
		}
	}
	...
	protected void onInit(Holo config, Bundle savedInstanceState) {
		...
		if (!config.ignoreApplicationInstanceCheck
				&& !(getApplication() instanceof Application)) {
			...
			if (getApplication().getClass() == android.app.Application.class) {
				...
				text += "Please sure that you extend "
		...
		if (this instanceof Activity) {
			...
			if (!config.ignoreThemeCheck
					&& ThemeManager.getThemeType(this) == ThemeManager.INVALID) {
	...
	private boolean requestDecorView(View view, LayoutParams params,
			int layoutRes) {
		if (mDecorView != null) {
			return true;
		}
		...
		final LayoutParams p = new LayoutParams(LayoutParams.MATCH_PARENT,
	...
	public abstract void setSupportProgressBarIndeterminate(
			boolean indeterminate);

	public abstract void setSupportProgressBarIndeterminateVisibility(
			boolean visible);
		...
		if (resid > ThemeManager._START_RESOURCES_ID) {
			...
			if ((resid & ThemeManager.COLOR_SCHEME_MASK) == 0) {
				...
				if (theme == 0) {
					...
					if (activity != null) {
						theme = ThemeManager.getTheme(activity.getIntent(),
								false);
					}
				}
